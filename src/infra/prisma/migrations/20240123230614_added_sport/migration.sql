/*
  ...
  - Added the required column `score` to the `Place` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "Sport" (
    ...
    CONSTRAINT "Sport_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
